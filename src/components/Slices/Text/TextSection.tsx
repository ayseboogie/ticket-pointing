...
export type TextSectionProps = {
  slice: Content.TextSliceTextSection;
};

const TextSection = ({ slice }: TextSectionProps) => {
  return (
    <section
      data-slice-variation={slice.variation}
      className="max-w-4xl px-6 mx-auto flex flex-col gap-4"
    >
      <PrismicRichText
        field={slice.primary.text}
        components={{
          heading2: ({ children }) => (
            <h2 className="mt-12 mb-6 font-display text-3xl tracking-tight text-darkGrey sm:text-4xl">
              {children}
            </h2>
          ),
          heading3: ({ children }) => (
            <h3 className="mt-10 mb-6 font-display text-2xl tracking-tight text-darkGrey sm:text-3xl">
              {children}
            </h3>
          ),
          heading4: ({ children }) => (
            <h4 className="mt-10 mb-4 font-display text-xl tracking-tight text-darkGrey sm:text-2xl">
              {children}
            </h4>
          ),
          heading5: ({ children }) => (
            <h5 className="mt-10 mb-4 font-display text-lg tracking-tight text-darkGrey sm:text-xl">
              {children}
            </h5>
          ),
          heading6: ({ children }) => (
            <h6 className="mt-10 mb-4 font-display text-md tracking-tight text-darkGrey sm:text-lg">
              {children}
            </h6>
          ),
          paragraph: ({ children }) => (
            <p className="mb-4 prose-lg tracking-tight text-darkGrey">
              {children}
            </p>
          ),
          preformatted: ({ node }) => (
